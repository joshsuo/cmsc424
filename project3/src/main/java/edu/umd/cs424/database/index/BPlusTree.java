...
public class BPlusTree implements Closeable {
	...
	private class BPlusTreeIterator implements Iterator<RecordId> {
		 ...
		 * iterates through every record in the tree in ascending order of the keys.
		 * <p>
		...
		public BPlusTreeIterator(BaseTransaction transaction) {
			...
			//this(transaction, null, null, -1, null);
		...
		public BPlusTreeIterator(BaseTransaction transaction,
			...
			this(transaction, lowerBound, upperBound, -1, null);
		...
		public BPlusTreeIterator(BaseTransaction transaction,
			...
			if((lowerBound == null || upperBound == null) && limit == -1 && filter == null) {
				this.curNode = root.getLeftmostLeaf(transaction);
				this.curIterator = curNode.scanAll();

			}
			else if (limit == -1 && filter == null) {
				//this.curNode = root.getLeftmostLeaf(transaction);
				this.curNode = root.get(transaction, lowerBound);
				this.curIterator = curNode.scanAll();
				//this.curNode.getKeys();
				//keys = 
